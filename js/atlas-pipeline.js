 ...
 */
async function runAtlasPipeline(atlas, srcCanvas, signal) {
  ...
  for (let i = 0; i < atlas.pipeline.length; i++) {
    ...
    if (!runner) continue;
    const ctx = { region: null, state, block: b, ai: null, signal: signal ?? null };
    b._running = true;
    _refreshAtlasBlockRunningState(i, true);
    try {
      const out = await runner(canvas, b.params, ctx);
      if (signal?.aborted) { b._running = false; _refreshAtlasBlockRunningState(i, false); return null; }
    ...
    b._running = false;
    _refreshAtlasBlockRunningState(i, false);
  }
  return canvas;
}
